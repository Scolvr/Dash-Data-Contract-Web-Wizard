  ...
  function updateExportScreenUI() {
    ...
    if (missingSteps.length > 0) {
      ...
      try {
        ...
        // Validate contract - use strict schema validation for tokens
        const hasTokens = !!contractJSON.tokens && Object.keys(contractJSON.tokens).length > 0;
        ...
        if (hasTokens) {
          ...
          if (window.ContractSchema && typeof window.ContractSchema.validateContract === 'function') {
            console.log('[Export] Using strict schema validation');
            ...
            console.log('[Export] Using fallback validation (ContractSchema not loaded)');
            const tokenConfig = contractJSON.tokens?.[0] || contractJSON.tokens?.['0'];
            if (!tokenConfig) {
              throw new Error('Token configuration missing');
            }
        ...
        } else if (window.EvoSDK && window.EvoSDK.DataContract) {
